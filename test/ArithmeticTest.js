...
function updateExoswapSegment(index, min, max) {
  let config = {};
  config.min = min;
  config.max = max;
  ExoswapSegment[index] = config;
}
...
function findSegmentMinPlayer(segIndex) {
  let firstMinAmount = ExoswapSegment[segIndex].max;
  let secondMinAmount = ExoswapSegment[segIndex].max;
  ...
  function doLeave(playerId,segIndex) {
    ...
    ExoswapSegment[segIndex].min = secondMinAmount<playerAmount?secondMinAmount:playerAmount;
  ...
  function joinMid(playerId) {
    ...
    ExoswapSegment[segIndex].max = ExoswapSegment[segIndex+1].min;
  }
  ...
  function join(name, amount) {
    ...
    for (let i = 1; i <= high; i++) {
      if (amount < ExoswapSegment[i].max) {
        segIndex = i;
        break;
      }
    }
    ...
    if (segIndex == 0) {
      ExoswapSegment[high].max = amount;
      segIndex = high;
    }
  ...
  updateRuler(10000, updateExoswapSegment);
...
// updateRuler(100, updateExoswapSegment);
